...
import { type ReleaseDateManager } from "./releaseDateManager";
...
describe("loggingReleaseDateManager", () => {
  ...
  beforeEach(() => {
    instance = {
      createReleaseDate: vi.fn(),
      deleteReleaseDate: vi.fn(),
      getReleaseDate: vi.fn(),
      updateReleaseDate: vi.fn(),
    };
    logger = {
      info: vi.fn(),
      error: vi.fn(),
  ...
  describe("createReleaseDate", () => {
    it("returns the result from the decorated instance", async () => {
      vi.mocked(instance.createReleaseDate).mockResolvedValue("29-06-2022");
    ...
    it("logs a success message", async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "Totalmobile release date set to 24-01-1988 for DST211Z by rich",
      );
      expect(logger.error).not.toHaveBeenCalled();
    });

    it("logs an error message", async () => {
      vi.mocked(instance.createReleaseDate).mockRejectedValue("foo");

      await expect(decorator.createReleaseDate("DST211Z", "29-06-2022")).rejects.toEqual("foo");
      expect(logger.error).toHaveBeenCalledWith(
        "Failed to set TM release date to 29-06-2022 for questionnaire DST211Z (user: rich)",
      );
  ...
  describe("deleteReleaseDate", () => {
    ...
    it("logs a message", async () => {
      await decorator.deleteReleaseDate("DST211Z");
      expect(logger.info).toHaveBeenCalledWith(
        "Totalmobile release date deleted for DST211Z by rich",
      );
      expect(logger.error).not.toHaveBeenCalled();
    });

    it("logs a message with previous release date exists for questionnaire", async () => {
      vi.mocked(instance.getReleaseDate).mockResolvedValue({
      ...
      await decorator.deleteReleaseDate("DST211Z");
      expect(logger.info).toHaveBeenCalledWith(
        "Totalmobile release date deleted (previously 2022-06-27) for DST211Z by rich",
      );
      expect(logger.error).not.toHaveBeenCalled();
    });

    it("logs a success message without previous release date if different questionnaire fetched", async () => {
      vi.mocked(instance.getReleaseDate).mockResolvedValue({ tmreleasedate: "2022-12-31" });
    ...
    it("logs an error message", async () => {
      vi.mocked(instance.deleteReleaseDate).mockRejectedValue("foo");

      await expect(decorator.deleteReleaseDate("DST211Z")).rejects.toEqual("foo");
      expect(logger.error).toHaveBeenCalledWith(
        "Failed to remove TM release date for questionnaire DST211Z (user: rich)",
      );
  ...
  describe("getReleaseDate", () => {
    it("returns the result from the decorated instance", async () => {
      vi.mocked(instance.getReleaseDate).mockResolvedValue("29-06-2022");
  ...
  describe("updateReleaseDate", () => {
    it("returns the result from the decorated instance", async () => {
      vi.mocked(instance.updateReleaseDate).mockResolvedValue("29-06-2022");
    ...
    it("logs a success message", async () => {
      await decorator.updateReleaseDate("DST211Z", "24-01-1988");
      expect(logger.info).toHaveBeenCalledWith(
        "Totalmobile release date updated to 24-01-1988 for DST211Z by rich",
      );
      expect(logger.error).not.toHaveBeenCalled();
    });

    it("logs an error message", async () => {
      vi.mocked(instance.updateReleaseDate).mockRejectedValue("foo");

      await expect(decorator.updateReleaseDate("DST211Z", "2022-06-29")).rejects.toEqual("foo");
      expect(logger.error).toHaveBeenCalledWith(
        "Failed to set TM release date to 2022-06-29 for questionnaire DST211Z (user: rich)",
      );
    ...
    it("logs a message with previous release date exists for questionnaire", async () => {
      vi.mocked(instance.getReleaseDate).mockResolvedValue({
        tmreleasedate: "2022-12-31T16:29:00+00:00",
      });

      await decorator.getReleaseDate("DST211Z");
      await decorator.updateReleaseDate("DST211Z", "2022-06-29");
      expect(logger.info).toHaveBeenCalledWith(
        "Totalmobile release date updated to 2022-06-29 (previously 2022-12-31) for DST211Z by rich",
      );
      expect(logger.error).not.toHaveBeenCalled();
    });
    ...
    it("logs a success message without previous release date if different questionnaire fetched", async () => {
      ...
      await decorator.updateReleaseDate("DST211Z", "2022-06-29");
      expect(logger.info).toHaveBeenCalledWith(
        "Totalmobile release date updated to 2022-06-29 for DST211Z by rich",
      );
      expect(logger.error).not.toHaveBeenCalled();
    });
  });
