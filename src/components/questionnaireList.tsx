...
const QuestionnaireList = ({ setErrored }: Props): ReactElement => {
  ...
  const filterQuestionnaireList = (questionnaireList: Questionnaire[], filterValue: string) => {
    ...
    const newFilteredList = questionnaireList.filter((questionnaire) => {
      if (filterValue === "") {
        return (
          questionnaire.name.toUpperCase().includes(filterValue.toUpperCase()) &&
          !isHiddenQuestionnaire(questionnaire.name)
        );
      }
  ...
  const getQuestionnairesList = useCallback(async () => {
    let questionnaires: Questionnaire[];

    try {
      questionnaires = await getQuestionnaires();
      clientLogger.info(
        `Response from get all questionnaires successful, data list length ${questionnaires.length}`,
      );
    } catch {
