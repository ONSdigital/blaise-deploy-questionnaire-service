...
describe("Deploy form summary page", () => {
  ...
  const validFiles = [
    ...
    {
      file: new File(["(♥_♥)"], `${frsQuestionnaireName}.bpkg`, {
        type: "application/pdf",
        lastModified: lastModified,
      }),
      questionnaire: questionnaireWithName(frsQuestionnaireName),
    },
  ];
  ...
  it("should display the telephone operation start date", async () => {
    ...
    expect(
      getByText(/Set a telephone operations start date for questionnaire?/i),
    ).toBeInTheDocument();
  });
  ...
  it("should not display the totalmobile release date for non-LMS, non-FRS questionnaires", async () => {
    ...
    expect(
      queryByText(/Set a totalmobile release date for questionnaire?/i),
